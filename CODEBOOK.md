# Reference data
activity - Activity label taken from the 'activity_labels.txt' file. This is linked to the 'y_test.txt' and 'y_train.txt' data.
subject_id - Subject ID taken from the 'subject_test.txt' and 'subject_train.txt' files.

# Measurement data
_Measurement data is taken from the 'X_test.txt' and 'X_train.txt' data files. Only those measurements that contain 'mean' of 'std' are in included in the output. The 'average' of each of measure is computed for each subject/activity pair._
